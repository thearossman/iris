...
impl TcpFlow {
    ...
    pub(super) fn insert_segment<T: Trackable>(
        ...
        if let Some(next_seq) = self.next_seq {
            ...
            // The SYN/ACK responding to the originator's SYN -- or, if that was
            // never observed, whatever ACK-bearing packet arrives in its place.
            //
            // The guard stays deliberately loose. Narrowing it to `SYN` alone is
            // tempting, since only a SYN truly pins the ISN, but then a flow whose
            // SYN/ACK was lost on the capture path never anchors and every segment
            // it sends buffers until `max_out_of_order` is reached -- at line rate,
            // `max_out_of_order` mbufs pinned per missed SYN/ACK. Anchoring on the
            // first ACK we see costs at most a small constant error in `next_seq`
            // and keeps the flow's memory flat.
            //
            // `seq_after` is what makes this safe: it charges the extra sequence
            // number only when SYN is actually set, so a bare ACK anchors at its own
            // sequence number rather than one past it. That phantom `+1` -- charging
            // for a SYN that was not there -- was the real defect here, not the
            // breadth of the guard.
            let expected_seq = seq_after(cur_seq, length, segment.flags());
            self.next_seq = Some(expected_seq);
            self.consumed_flags |= segment.flags();
            self.last_ack = Some(segment.ack_no());
            ...
            // Neither SYN nor ACK on a flow with no sequence origin yet. Rare in
            // practice -- nearly every segment after a handshake carries ACK -- so
            // this arm stays close to unreachable, which is what keeps an unanchored
            // flow from accumulating buffered segments.
            self.buffer_ooo_seg(segment, info, subscription, registry);
        }
    }
...
mod tests {
    use super::*;
    // A flag the reassembly logic itself never has to name.
    use crate::protocols::packet::tcp::PSH;
