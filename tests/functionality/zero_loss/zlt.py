...
def execute(cmd, executable):
    ...
    for stdout_line in iter(popen.stdout.readline, ''):
        ...
        # ---- THROUGHPUT PARSING ----
        # We look for the summary line:
        # AVERAGE Process: 72.0577 Gbps / 19.0438 Mpps
        if 'AVERAGE Process' in stdout_line and 'Gbps' in stdout_line:
            m = re.search(r'AVERAGE Process:\s*([0-9.eE+-]+)\s*Gbps', stdout_line)
            if m:
                gbps_val = float(m.group(1))
                throughput = gbps_val * 1e9  # store in bps, same units as original script
        # ---- END THROUGHPUT PARSING ----

        if 'SW Dropped' in stdout_line:
            num = re.findall(r'\d*\.*\d+\%', stdout_line)
            if not num:
                continue
...
def main(args):
    ...
    executable = f'/home/aliya/retina/target/release/{binary}'
